...
exports.index = function (req, res, next) {
  ...
  User.getUserByLoginName(user_name, function (err, user) {
    ...
    if (!user) {
      res.render('notify/notify', {error: ' Thành viên này không tồn tại 。'});
      return;
    }

    var render = function (recent_topics, recent_replies) {
      user.friendly_create_at = tools.formatDate(user.create_at, true);
      //  Nếu người dùng không hoạt động ， Sau đó, người quản trị có thể giúp kích hoạt 
      var token = '';
      ...
      res.render('user/index', {
        ...
        token: token,
        pageTitle: util.format('@%s  Trang cá nhân ', user.loginname),
      });
    };
...
exports.setting = function (req, res, next) {
  ...
  //  Hiển thị một thông báo lỗi hoặc thành công 
  ...
  if (action === 'change_password') {
    ...
    if (!old_pass || !new_pass) {
      return res.send(' Các mật khẩu cũ hoặc mật khẩu mới không thể để trống ');
    }

    User.getUserById(req.session.user._id, ep.done(function (user) {
      tools.bcompare(old_pass, user.pass, ep.done(function (bool) {
        if (!bool) {
          return showMessage(' Mật khẩu hiện nay là không chính xác 。', user);
        }
        ...
        tools.bhash(new_pass, ep.done(function (passhash) {
          ...
          user.save(function (err) {
            ...
            return showMessage(' Mật khẩu đã được sửa đổi 。', user, true);
...
exports.list_replies = function (req, res, next) {
  ...
  User.getUserByLoginName(user_name, function (err, user) {
    ...
    Reply.getRepliesByAuthorId(user._id, opt, proxy.done(function (replies) {
      //  Nhận tất cả các chủ đề đã nhận xét 
